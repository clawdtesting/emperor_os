...
async function seedAssignedStateFromJob(jobId, job, specPayload) {
  ...
  const patch = {
    ...
    contractVersion: String(job?.source || '').toLowerCase() === 'agijobmanager-v2' ? 'v2' : 'v1',
    status: 'assigned',
...
main().catch((err) => {
  emit('fatal', 'run_assigned_job_pipeline', 'error', err.message)
  console.error(`[assigned-job] fatal: ${err.message}`)
  process.exit(1)
})
