...
export async function generateApplicationContent({ procurementId, jobSpec, forceFallback = false }) {
  ...
  let retrieval = null;
  try {
    retrieval = await createRetrievalPacket({ procurementId: id, phase: "application", keywords });
  } catch (err) {
    log(`Retrieval search failed for #${id}: ${err.message}`);
  }
...
export async function generateTrialContent({ procurementId, jobSpec, trialPlan, forceFallback = false }) {
  ...
  let retrieval = null;
  try {
    retrieval = await createRetrievalPacket({ procurementId: id, phase: "trial", keywords });
  } catch (err) {
    log(`Retrieval search failed for #${id}: ${err.message}`);
  }
...
function buildCompletionTemplate({ procurementId, linkedJobId, spec, executionResult }) {
  const title = spec?.title ?? spec?.properties?.title ?? "Untitled Job";
