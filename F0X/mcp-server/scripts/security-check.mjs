...
function run() {
  ...
  mustContain(
    ...
    /token valid pre-logout|logout invalidation verified/,
    'live check verifies token works pre-logout'
  );
  mustContain(
    'scripts/security-live-check.mjs',
    /token invalid post-logout|revocation|post-logout token expected denied/,
    'live check verifies token is invalid post-logout (revocation)'
  );
  mustContain(
    'scripts/security-live-check.mjs',
    /SECURITY_TEST_SKIP_LOGOUT|SECURITY_TEST_VERIFY_LOGOUT/,
    'live check has explicit guard for logout revocation test'
  );
