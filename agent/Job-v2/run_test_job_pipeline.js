...
function parseArgs(argv) {
  const out = {
    jobMd: path.join(ROOT, "AGIJobManager-v2-test-job.md"),
    jobId: "v2_990001",
    withSubmit: true,
  };
...
function parseTestJobMarkdown(raw) {
  ...
  const title = lines[3] || "AGIJobManager v2 Test Job";
  ...
  const details = [
    "Test execution from AGIJobManager-v2-test-job.md",
    title,
    "",
    "Deliverables:",
    ...deliverables,
...
async function main() {
  ...
  console.log(`[v2-test] seeded assigned state for ${args.jobId}`);
  ...
  if (args.withSubmit) {
    ...
    console.log("[v2-test] --skip-submit enabled: stopping after validate stage");
  }

  console.log(`[v2-test] complete for ${args.jobId}`);
}

main().catch((err) => {
  console.error(`[v2-test] fatal: ${err.message}`);
  process.exit(1);
});
